...
export function useDataIterator(modelName, options = {}) {
    ...
    const fields = computed(() => {
        ...
        if (
            isSortable.value &&
            sortableField.value &&
            !allFields.includes(sortableField.value)
        ) {
    ...
    const toggleSort = (field) => {
        ...
        if (currentSort) {
            const [currentField, currentDirection = "asc"] =
                currentSort.split(":");
    ...
    const getSortDirection = (field) => {
        ...
        const sortItem = orderBy.value.find((item) =>
    ...
    const resequenceWithState = async (ids) => {
        try {
            ...
            toast.error(
                formatValidationErrors(err, t("Échec de la mise à jour de l'ordre"))
            );
            throw err;
        } finally {
            loading.value = false;
