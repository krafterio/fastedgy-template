<template>
  <div :class="cn('flex items-center flex-row-reverse', $attrs.class)">
    ...
    <Avatar v-if="remainingCount > 0" class="-ml-2 hover:z-10 relative ring-2 ring-background cursor-pointer">
      <AvatarFallback class="bg-muted-foreground text-white"> +{{ remainingCount }} </AvatarFallback>
    </Avatar>
...
const remainingCount = computed(() => {
  return props.max && props.avatars.length > props.max ? props.avatars.length - props.max : 0;
});
