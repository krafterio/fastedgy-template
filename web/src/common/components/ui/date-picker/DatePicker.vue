<template>
  <Popover>
    <PopoverTrigger as-child>
      <Button
        variant="outline"
        :class="cn('w-full justify-start text-left font-normal', !internalModelValue && 'text-muted-foreground')"
      >
      ...
      >
        <CalendarHeader>
          <CalendarHeading class="flex w-full items-center justify-between gap-2">
            <Select
              :default-value="placeholderRef.month.toString()"
              @update:model-value="
                (v) => {
            ...
            <Select
              :default-value="placeholderRef.year.toString()"
              @update:model-value="
                (v) => {
                  if (!v || !placeholderRef) return;
