<template>
  <div class="space-y-4">
    <div
      v-if="$slots.filters || $slots.actions || sortOptions.length > 0"
      ...
      <div class="flex items-center gap-2">
        <DropdownMenu v-if="uniqueSortFields.length > 0">
          ...
          <DropdownMenuContent align="end">
            <DropdownMenuItem v-for="field in uniqueSortFields" :key="field.field" @click="toggleSort(field.field)">
    ...
    <component
      v-else
      :is="isSortable ? VueDraggable : 'div'"
      v-bind="
        isSortable
          ? {
              modelValue: items,
        ...
        <Card class="overflow-hidden cursor-pointer pt-4 h-full" @click="$emit('item-click', item)">
          ...
          <DropdownMenu v-if="$slots['item-actions']">
            <DropdownMenuTrigger as-child @click.stop>
              <Button variant="ghost" size="icon" class="absolute top-3 right-3 bg-background shadow-sm h-8 w-8">
