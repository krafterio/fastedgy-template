  ...
  <div class="space-y-4">
    ...
    <div v-else class="rounded-md border overflow-auto min-h-0" :class="tableClass">
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead v-if="isSelectionEnabled" class="w-[50px]">
              <Checkbox
                :model-value="selection.isAllVisibleSelected"
                @update:modelValue="
                  () => (selection.isAllVisibleSelected ? selection.clear() : selection.selectAllVisible())
                "
              />
            </TableHead>
            ...
            <TableHead
              ...
              :class="[
                isSortableColumn(column) &&
                  props.orderable !== false &&
                  'cursor-pointer hover:bg-muted/50 select-none group',
            ...
            >
              <div class="flex items-center gap-2">
                ...
                <ArrowUpDown
                  ...
                  :class="['h-4 w-4 transition-opacity opacity-0 group-hover:opacity-50']"
                />
                ...
                <ArrowUp
                  v-if="
                    isSortableColumn(column) &&
                    props.orderable !== false &&
                    getSortDirection(getSortPath(column)) === 'asc'
                  "
                  :class="['h-4 w-4 transition-colors text-muted-foreground group-hover:text-primary']"
                />
                <ArrowDown
                  v-if="
                    isSortableColumn(column) &&
                    props.orderable !== false &&
                    getSortDirection(getSortPath(column)) === 'desc'
                  "
                  :class="['h-4 w-4 transition-colors text-muted-foreground group-hover:text-primary']"
                />
              </div>
        ...
        <component
          :is="isSortable ? VueDraggable : TableBody"
          v-bind="
            isSortable
              ? {
                  modelValue: items,
              ...
              : {}
          "
          :class="isSortable && '[&_tr]:border-b'"
          @update:modelValue="items = $event"
        ...
        >
          ...
          >
            ...
            <TableCell v-if="isSelectionEnabled" @click.stop>
              <Checkbox :model-value="selection.has(row.id)" @update:modelValue="() => selection.toggle(row.id)" />
            </TableCell>
            ...
            >
              ...
              <slot
                :name="`cell-${column.key.replace(/\./g, '__')}`"
                :row="row"
                ...
                <DropdownMenuContent align="end">
                  <template v-for="(action, index) in actions" :key="index">
                    ...
                    <DropdownMenuItem
                      v-else
                      @click.stop="
                        action.handler ? action.handler(row) : $emit('row-action', { action: action.action, row })
                      "
    ...
    <DataTablePagination
      ...
      :selection="isSelectionEnabled ? selection : null"
    />
